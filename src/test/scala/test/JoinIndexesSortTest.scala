...
//          2 -> Seq(atom3.hash, atom5.hash)),
    ...
    val (newBody, hash) = JoinIndexes.presortSelect(a => sm.getEDBResult(a.rId).size, jIdx, sm)
...
//    rule(x) :- (a5, a2, a4, a6, a0, a3, a1)
  ...
  test("worst sort with repeated vars") {
    ...
    val (newBody, hash) = JoinIndexes.presortSelectWorst(a => sm.getEDBResult(a.rId).size, jIdx, sm)
