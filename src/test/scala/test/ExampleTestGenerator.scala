...
//           blocking
          case "JITStaged_Sel_RULE_Block_QuotesIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.RULE, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Quotes)))
          case "JITStaged_Sel_ALL_Block_QuotesIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.ALL, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Quotes)))
          case "JITStaged_Sel_DELTA_Block_QuotesIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.DELTA, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Quotes)))
          case "JITStaged_Sel_RULE_Block_BCIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.RULE, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Bytecode)))
          case "JITStaged_Sel_ALL_Block_BCIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.ALL, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Bytecode)))
          case "JITStaged_Sel_DELTA_Block_BCIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.DELTA, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Bytecode)))
          case "JITStaged_Sel_RULE_Block_LambdaIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.RULE, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Lambda)))
          case "JITStaged_Sel_ALL_Block_LambdaIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.ALL, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Lambda)))
          case "JITStaged_Sel_DELTA_Block_LambdaIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.DELTA, dotty = dotty, compileSync = CompileSync.Blocking, sortOrder = SortOrder.VariableR, backend = Backend.Lambda)))


          // async
          case "JITStaged_Sel_RULE_Async_QuotesIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.RULE, dotty = dotty, compileSync = CompileSync.Async, sortOrder = SortOrder.VariableR, backend = Backend.Quotes)))
          case "JITStaged_Sel_ALL_Async_QuotesIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.ALL, dotty = dotty, compileSync = CompileSync.Async, sortOrder = SortOrder.VariableR, backend = Backend.Quotes)))
          case "JITStaged_Sel_RULE_Async_BCIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.RULE, dotty = dotty, compileSync = CompileSync.Async, sortOrder = SortOrder.VariableR, backend = Backend.Bytecode)))
          case "JITStaged_Sel_ALL_Async_BCIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.ALL, dotty = dotty, compileSync = CompileSync.Async, sortOrder = SortOrder.VariableR, backend = Backend.Bytecode)))
          case "JITStaged_Sel_RULE_Async_LambdaIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.RULE, dotty = dotty, compileSync = CompileSync.Async, sortOrder = SortOrder.VariableR, backend = Backend.Lambda)))
          case "JITStaged_Sel_ALL_Async_LambdaIndexed" =>
            Program(StagedExecutionEngine(IndexedStorageManager(), JITOptions(mode = Mode.JIT, granularity = Granularity.ALL, dotty = dotty, compileSync = CompileSync.Async, sortOrder = SortOrder.VariableR, backend = Backend.Lambda)))
