...
class TyQLExecutionEngine(override val storageManager: StorageManager,
  ...
  def toCaracIR(tyqlAST: DatabaseAST[?])(using ctx: TyQLInterpreterContext): (IROp[?], IROp[EDB]) =
        ...
        storageManager match
          case ddb: DuckDBStorageManager =>
            if linear then
              ...
              val program = TyQLToIROp().generateTopLevelProgram(ruleMap, true)
  ...
  def solveTyQL(tyqlAST: DatabaseAST[?]): Set[Seq[StorageTerm]] =
...
//    debug("", () => s"solve $rId with options $defaultJITOptions")
    ...
    storageManager match
      case ddb: DuckDBStorageManager =>
        ...
        val initCtx = TyQLInterpreterContext(ddb, () => ???)
        val (irTree, finalNode) = toCaracIR(tyqlAST)(using initCtx)
        ddb.initEvaluation()

          val irCtx = TyQLInterpreterContext(ddb, () => finalNode.run(ddb).asInstanceOf[DuckDBEDB].execute_toSetOfSeq())
