...
class Printer[S <: StorageManager](val s: S) {
  ...
  def printIR[T](node: IROp[T], ident: Int = 0, seq: Int = 0)(using ctx: InterpreterContext): String = {
    ...
    i + (node match {
      ...
      case UnionSPJOp(rId, hash, children:_*) => s"UNION_SPJ::${ctx.storageManager.ns(rId)}::${s.asInstanceOf[CollectionsStorageManager].allRulesAllIndexes(rId)(hash)}::${children.map(o => printIR(o, ident+1)).mkString("(\n", ",\n", ")")}"
