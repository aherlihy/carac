...
import test.examples.tastyslistlibnegated.tastyslistlibnegated as tastyslistlibnegated_test
...
class tastyslistlibnegated() extends ExampleBenchmarkGenerator("tastyslistlibnegated") with tastyslistlibnegated_test {
...
//  }
//
//  @Benchmark def jit_EVALRULEBODY_aot_async_bestsel13__(blackhole: Blackhole): Unit = {
//    val p = s"${Thread.currentThread.getStackTrace()(2).getMethodName.split("__").head}"
//    if (!programs.contains(p))
//      throw new Exception(f"Error: program for '$p' not found")
//    blackhole.consume(run(programs(p), result))
//  }
//
//  @Benchmark def jit_EVALRULEBODY_aot_async_bestsel1__(blackhole: Blackhole): Unit = {
//    val p = s"${Thread.currentThread.getStackTrace()(2).getMethodName.split("__").head}"
//    if (!programs.contains(p))
//      throw new Exception(f"Error: program for '$p' not found")
//    blackhole.consume(run(programs(p), result))
