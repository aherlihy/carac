...
#echo "Profiling then compiling souffle on program $1"
...
OUTDIR="./souffle-out/profile-compile/$1"
...
$SOUFFLE/souffle -c --profile=$1-profile-compile --emit-statistics -F $FACTDIR $PROGRAM -D $OUTDIR --jobs=1  --wno=var-appears-once --wno=no-rules-nor-facts
$SOUFFLE/souffle -c --a=$1-profile-compile -F $FACTDIR $PROGRAM -D $OUTDIR --jobs=1  --wno=var-appears-once --wno=no-rules-nor-facts
