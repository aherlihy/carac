...
<<<<<<< HEAD:bench/src/test/scala/datalog/benchmarks/examples/tastyslistlib_optimized.scala
...
>>>>>>> 4d2bffe (Add an experimental IROp to bytecode compiler):bench/src/test/scala/datalog/benchmarks/examples/tastyslistlibinverse_autobestmixed.scala
    val p = s"${Thread.currentThread.getStackTrace()(2).getMethodName.split("__").head}"
    if (!programs.contains(p))
      throw new Exception(f"Error: program for '$p' not found")
    blackhole.consume(run(programs(p), result))
